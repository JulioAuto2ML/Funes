...
| `test_cron_tool` | schedule_job validation, CRUD roundtrip, run_job_now, `kind="shell"` requires the owner's `execute_shell` permission at schedule and fire time, and `kind="script"`: scheduling is refused for an ungranted script, a scheduled script runs with no shell enabled, and revoking either the agent's grant or the account's permission stops an existing job from firing |
| `test_file_tools` | fs_guard traversal prevention, read/write/append, PDF extraction, image detection |
| `test_shell_tool` | Disabled-by-default check, enabled path, and the child environment is an allowlist (no service token or `funes.local` secret reaches `env`; `FUNES_CHILD_ENV` and manifest `env:` pass through) |
...
| `test_web_fetch` | Non-UTF-8 handling, large inline script, HTML landmark dropping, the resolving private-host guard (every spelling of loopback, RFC 1918, link-local, ULA, v4-mapped v6, unresolvable names), manual redirect following |
