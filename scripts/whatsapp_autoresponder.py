...
has no send capability of its own — only `recall`/`remember`. This script is
the only thing that ever calls the bridge's /api/send, and only for chat_jids
already on WHATSAPP_WHITELIST, always replying into the exact chat the
incoming message came from. The LLM never chooses a recipient.
"""
import json
import os
import re
...
def main():
    ...
    while True:
        ...
        for msg_id, chat_jid, sender, content, timestamp in new_messages:
            if chat_jid not in WHITELIST:
                continue
            if not content or not content.strip():
                continue  # media-only or empty message, nothing to reply to

            log(f"New message from {sender} in {chat_jid}: {content[:80]!r}")
            reply = ask_funes(sanitize_session(chat_jid), content)
